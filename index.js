...
function handleproduct(product ,productPass){
    ...
    let productUpdate = parseInt(phoneInput.value);
    //  let totalPrice = phoneUpdate + 1;
    let newProductUpdate = productUpdate;
    if(productPass == true){
    ...
    if(productPass == false && productUpdate > 0){
     ...
     phoneInput.value = newProductUpdate;
...
function totalPrice(){
   ...
   const caseCount = getInputValue('case');
    ...
    const totalallPrice = phoneCount * 1219 + caseCount * 60;
    document.getElementById('total-price').innerText = '$' + totalallPrice;

    const tax = Math.round(totalallPrice / 100*10);
    document.getElementById('tax-amount').innerText='$' + tax;

    const grandTotal = totalallPrice + tax;
    document.getElementById('grand-total').innerText='$' + grandTotal;

}

function getInputValue(productValue){
    const  productInput = document.getElementById(productValue+'-count');
